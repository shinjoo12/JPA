...
public class A_EntityManagerLifeCylcleTests {
    ...
    * application 스코프와 동일하게 싱글톤으로 생성해서 관리한느 것이 효율적이다.
    * 따라서 데이터베이슬르 사용하는 애플리케이션 당 한 개의 EntityManagerFactory를 생성한다.
    *
     *
    * */
    ...
    * 엔티티 매너저는 엔티티를 저장하는 메모리 상의 데이터베이스를 관리하는 인스턴스이다.
    * 엔티티를 저장하고, 수정, 삭제, 조회하는 등의 엔티티와 관련된 모든 일을 한다.
    * 엔티티 매너저는 thread-safe 하지 않기 때문에 동시성 문제가 발생할 수 있다.
