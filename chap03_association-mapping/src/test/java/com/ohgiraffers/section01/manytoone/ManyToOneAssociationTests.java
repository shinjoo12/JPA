...
public class ManyToOneAssociationTests {
    ...
    * */
     ...
     * 예를들어 하나의 카테고리가 여러 개의 메뉴를 가질 수 있는 상황에서 메뉴 엔티티가 카테고리 엔티티를 참조하는 것이다.
...
//        System.out.println("menuCategory" + menuCategory);
    ...
    /*
     * commit()을 할 경우 컨텍스트 내에 저장 된 영속성 객체를 insert하는 쿼리가 동작된다.
     * 단, 카테고리가 존재하는 값이 아니므로 부모 테이블(tbl_category)에 값이 먼저 들어있어야 그 카테고리를 참조해서
     * 자식 테이블(tbl_menu)에 데이터를 넣을 수 있다.
     * 영속성 전이란 특정 엔터티를 영속화할 때 연관된 엔티티도 함께 영속화 한다는 의미이다.
    ...
    void 다대일_연관관계_객체_삽입_테스트(){
        ...
        menuAndCategory.setMenuCode(5432);
        menuAndCategory.setMenuName("멸치빙수");
        menuAndCategory.setMenuPrice(123123);

        Category category = new Category();
        category.setCategoryCode(1231);
        category.setCategoryName("키키키");
        ...
        MenuAndCategory foundMenuAndCategory = entityManager.find(MenuAndCategory.class, 5432);
